...
백준에서 결과를 공백으로 구분된 문자열로 출력해야 할 때 `*` 언패킹을 사용하면 간단합니다.

```python
result = [1, 2, 3, 4, 5]

# ❌ 번거로운 방법
print(' '.join(map(str, result)))  # "1 2 3 4 5"

# ✅ * 언패킹 사용
print(*result)  # "1 2 3 4 5"
```

`print(*list)`는 리스트의 원소를 개별 인자로 전달하며, `print()`의 기본 구분자가 공백이기 때문에 자동으로 공백으로 이어집니다.

구분자를 바꾸고 싶다면 `sep` 인자를 사용합니다.

```python
print(*result, sep='\n')  # 줄바꿈으로 구분
print(*result, sep=', ')  # ", "로 구분
```
